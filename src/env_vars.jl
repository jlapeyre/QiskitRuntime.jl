...
const _var_docs =
    let  unused = "Currently unused by QiskitRuntime.jl", _var_descr = Dict(
...
function get_env(name::Symbol, default=nothing)
    name in _ENV_VARS || throw(ArgumentError(lazy"Environment variable `$name` is not used by QiskitRuntime.jl"))
...
"""
function env_vars()
    Dict{Symbol, Union{String, Nothing}}(var => get_env(var) for var in _ENV_VARS)
end
